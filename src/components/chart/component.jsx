...
function hourTransformAMPM(hour, params = {}) {
  ...
  if ((newHour || newHour === 0) && params.withAMPM) {
    newHour =
      hour < 12 || hour === 24
        ? `${hourTransform(hour)}AM`
        : `${hourTransform(hour - 12)}PM`;
...
function CustomizedTick(props) {
  ...
  return (
    <>
      ...
      >
        <tspan x={x} y={y}>
          {hourTransformAMPM(payload.value, {
            withAMPM: withHours,
            lowercase: true
          })}
        </tspan>
...
function getLabel(key, value, props = {}) {
  ...
  if (
    key === PERIOD_FUTURE_SEASONAL ||
    (props.isTooltip && key === PERIOD_HISTORICAL) ||
    props.isMonth
  ) {
    let month = '';
    if (date.getMonth() || date.getMonth() === 0) {
    ...
    <div className={styles['customTooltip']}>
      <span style={labelStyle}>
        {theme === THERMALCOMFORT ? (
          <>
            {showHours
              ? hourTransformAMPM(label, { withAMPM: true, lowercase: false })
      ...
      {payload &&
        payload.length > 0 &&
        payload
          .filter(item => !ignoreKeys.includes(item.name))
          .map(item => {
...
export const ThermalComfortChart = ({
  ...
  return (
    <div className={cx(styles['c-chart'])}>
      ...
      {period === 'historical' ? (
        <h4>
          Thermal stress events per month (averaged per geometry, data available
          from 2004)
        </h4>
      ) : (
        <h4>
          Thermal stress currently not available for the time period selected.
          <br />
          Please select <span>Historical</span>
        </h4>
      )}
      {period === 'historical' && (
          ...
          <ResponsiveContainer width="100%" height={220}>
            ...
            >
              ...
              <YAxis
                ...
                allowDecimals={false}
                tickFormatter={tick =>
                  tick !== 0 ? tick.toFixed(2) : tick.toFixed()
                }
              />
              <Tooltip
                itemStyle={{
              ...
              <Legend
                ...
                chartHeight={32}
                content={_p =>
                  legendContent({ payload: sortBy(_p.payload, 'color') })
                }
              />
            </AreaChart>
          </ResponsiveContainer>
  ...
  return (
    <div className={cx(styles['c-chart'])}>
      ...
      <div className={styles['c-chart-inside']}>
        ...
        <div
          ref={chartBox}
          className={cx(styles['c-chart-slider-container'], styles.withPadding)}
        >
          <Slider
            coordinates={coordinates}
            setCoordinates={setCoordinates}
...
export const TemperatureChart = ({
  ...
  const filteredData = (timeFilter.from && timeFilter.to
    ? data.filter(d => {
        const time = new Date(d.time).getTime();
        return time >= timeFilter.from && time <= timeFilter.to;
      })
...
export const ClimatologyChart = ({
  ...
  return (
    <div
      className={cx(styles['c-chart'], styles.withPadding, styles.climatology)}
    >
      <div className={styles.info} onClick={iconClickAfter}>
        <Icon name="icon-info" />
      </div>
      ...
      <div className={styles['c-chart-inside']}>
        <ResponsiveContainer width="100%" height={530}>
          ...
          >
            ...
            />
            {barsList.map(bar => (
              <Bar stackId="a" key={bar.dataKey} {...bar} />
            ))}
            <Legend
              layout="horizontal"
              verticalAlign="bottom"
...
export const ThermalComfortMainChart = ({
  data = [],
  iconClickAfter = () => {}
}) => {
  return (
    ...
    >
      ...
      {data.length > 0 ? (
        ...
        <div className={styles.noData}>
          There is currently no data available for this selection. Please select
          Madrid, Bizkaia or Gipuzkoa to see PET values.
        </div>
      )}
    </div>
  );
